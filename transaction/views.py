...
# Manpower Booking
class ManpowerBookingList(APIView):
    ...
    def get(self, request, pk = None):       
        ...
        try:
            if pk:
                ...
                serializeobj = serializer.ListManpowerBookingSerializer(list)
                return Response({"status":error.context['success_code'], "data": serializeobj.data}, status=status.HTTP_200_OK)
       
        except models.ManpowerBooking.DoesNotExist:
            return Response({"status" :error.context['error_code'], "message":"Manpower booking" +language.context[language.defaultLang]['dataNotFound']}, status = status.HTTP_200_OK)
        ...
        serializer = cSerializer.ListManpowerBookingSerializer(lists, many=True)
        print('serializer serializer',serializer.data)
        return Response({"status":error.context['success_code'], "data": serializer.data}, status=status.HTTP_200_OK)

class ManpowerBookingCRUD(APIView):
    def get_object(self, pk):
            try:
    ...
    def post(self,request, pk = None):
        ...
        if 'id' not in request.data:
            ...
            if not pk:
                ...
                request.data['created_ip'] = Common.get_client_ip(request)
                saveserialize = cSerializer.ManpowerBookingSerializer(data = request.data)

                if saveserialize.is_valid():
                    saveserialize.save()

                    return Response({"status" : error.context['success_code'], "message":"Manpower booking" +language.context[language.defaultLang]['insert'], "data":''}, status=status.HTTP_200_OK)
                else:
                    return Response({"status" :error.context['error_code'], "message":error.serializerError(saveserialize)}, status=status.HTTP_200_OK)
            else:
                ...
                saveserialize = cSerializer.ManpowerBookingSerializer(list, data = request.data, partial= True)                
                if saveserialize.is_valid():
                    saveserialize.save()
