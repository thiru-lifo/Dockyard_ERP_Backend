...
class ImportExcelEquipment(APIView):
    ...
    def post(self,request, pk = None):
        ...
        if excel_upload_obj:
            ...
            return Response({"status" :error.context['error_code'],"message":error.serializerError(saveserialize)}, status = status.HTTP_200_OK)
#CategoryType
class CategoryTypeViews(APIView):
    ...
    def get(self, request, pk = None):
        ...
        try:
            ...
            if pk:
                list = models.CategoryType.objects.filter(pk=pk).exclude(status='3').get()
                serializeobj = cSerializer.ListCategoryTypeSerializer(list)
                return Response({"status": error.context['success_code'], "data": serializeobj.data}, status=status.HTTP_200_OK)
       
        except models.CategoryType.DoesNotExist:
            return Response({"status" : error.context['error_code'], "message":"Status" +language.context[language.defaultLang]['dataNotFound'] }, status = status.HTTP_200_OK)

        lists = models.CategoryType.objects.exclude(status='3')
        if normal_values:
            lists = lists.filter(reduce(operator.and_, 
                               (Q(**d) for d in [dict([i]) for i in normal_values.items()])))
        if array_values:
        ...
        serializer = cSerializer.ListCategoryTypeSerializer(lists, many=True)
        return Response({"status":error.context['success_code'] , "data": serializer.data}, status=status.HTTP_200_OK)


class CategoryTypeDetailViews(APIView):

    def get_object(self,pk):

        try:
            return models.CategoryType.objects.get(pk = pk)
    ...
    def post(self,request, pk = None):
        ...
        if 'name' not in request.data and request.data['status'] != 3:
            ...
            if 'id' in request.data:
                ...
                if not pk: 
                    duplicate_code = models.CategoryType.objects.values('code').filter(code=request.data['code']).exclude(status=3)
                    duplicate_name = models.CategoryType.objects.values('name').filter(name=request.data['name']).exclude(status=3)
                    ...
                    if duplicate_code:            
                        ...
                        saveserialize = cSerializer.CategoryTypeSerializer(data = request.data)
                        if saveserialize.is_valid():
                            saveserialize.save()
                            return Response({"status" :error.context['success_code'], "message":"New CategoryType" +language.context[language.defaultLang]['insert'], "data":saveserialize.data}, status=status.HTTP_200_OK)
                        else:
                            return Response({"status" :error.context['error_code'], "message": error.serializerError(saveserialize)}, status=status.HTTP_200_OK)

                else:
                    if request.data['status'] != 3:
                        duplicate_code = models.CategoryType.objects.values('code').filter(code=request.data['code']).exclude(Q(id=request.data['id']) | Q(status=3))
                        duplicate_name = models.CategoryType.objects.values('name').filter(name=request.data['name']).exclude(Q(id=request.data['id']) | Q(status=3))
                        if duplicate_code:            
                            return Response({"status" :error.context['error_code'], "message":language.context[language.defaultLang]['exit code'] },status=status.HTTP_200_OK)                    
                        elif duplicate_name:   
                            return Response({"status" :error.context['error_code'], "message":language.context[language.defaultLang]['exit name'] },status=status.HTTP_200_OK)                                            
                    ...
                    saveserialize = cSerializer.CategoryTypeSerializer(list, data = request.data, partial= True)
                    if saveserialize.is_valid():
                        saveserialize.save()
                        return Response({"status" :error.context['success_code'], "message":"CategoryType" +language.context[language.defaultLang]['update'], "data":saveserialize.data}, status=status.HTTP_200_OK)
                    else:
                        return Response({"status" :error.context['error_code'], "message":error.serializerError(saveserialize)}, status = status.HTTP_200_OK)
            else: 
...
class PayScaleViews(APIView):
    ...
    def get(self, request, pk = None):
        ...
        try:
            ...
            if pk:
                list = models.PayScale.objects.filter(pk=pk).exclude(status='3').get()
                serializeobj = cSerializer.ListPayScaleSerializer(list)
                return Response({"status": error.context['success_code'], "data": serializeobj.data}, status=status.HTTP_200_OK)
       
        except models.PayScale.DoesNotExist:
            return Response({"status" : error.context['error_code'], "message":"Status" +language.context[language.defaultLang]['dataNotFound'] }, status = status.HTTP_200_OK)

        lists = models.PayScale.objects.exclude(status='3')
        if normal_values:
            lists = lists.filter(reduce(operator.and_, 
                               (Q(**d) for d in [dict([i]) for i in normal_values.items()])))
        if array_values:
        ...
        serializer = cSerializer.ListPayScaleSerializer(lists, many=True)
        return Response({"status":error.context['success_code'] , "data": serializer.data}, status=status.HTTP_200_OK)


class PayScaleDetailViews(APIView):

    def get_object(self,pk):

        try:
            return models.PayScale.objects.get(pk = pk)
    ...
    def post(self,request, pk = None):
        ...
        if 'name' not in request.data and request.data['status'] != 3:
            ...
            if 'id' in request.data:
                ...
                if not pk: 
                    duplicate_code = models.PayScale.objects.values('code').filter(code=request.data['code']).exclude(status=3)
                    duplicate_name = models.PayScale.objects.values('name').filter(name=request.data['name']).exclude(status=3)
                    ...
                    if duplicate_code:            
                        ...
                        saveserialize = cSerializer.PayScaleSerializer(data = request.data)
                        if saveserialize.is_valid():
                            saveserialize.save()
                            return Response({"status" :error.context['success_code'], "message":"New PayScale" +language.context[language.defaultLang]['insert'], "data":saveserialize.data}, status=status.HTTP_200_OK)
                        else:
                            return Response({"status" :error.context['error_code'], "message": error.serializerError(saveserialize)}, status=status.HTTP_200_OK)

                else:
                    if request.data['status'] != 3:
                        duplicate_code = models.PayScale.objects.values('code').filter(code=request.data['code']).exclude(Q(id=request.data['id']) | Q(status=3))
                        duplicate_name = models.PayScale.objects.values('name').filter(name=request.data['name']).exclude(Q(id=request.data['id']) | Q(status=3))
                        if duplicate_code:            
                            return Response({"status" :error.context['error_code'], "message":language.context[language.defaultLang]['exit code'] },status=status.HTTP_200_OK)                    
                        elif duplicate_name:   
                            return Response({"status" :error.context['error_code'], "message":language.context[language.defaultLang]['exit name'] },status=status.HTTP_200_OK)                                            
                    ...
                    saveserialize = cSerializer.PayScaleSerializer(list, data = request.data, partial= True)
                    if saveserialize.is_valid():
                        saveserialize.save()
                        return Response({"status" :error.context['success_code'], "message":"PayScale" +language.context[language.defaultLang]['update'], "data":saveserialize.data}, status=status.HTTP_200_OK)
                    else:
                        return Response({"status" :error.context['error_code'], "message":error.serializerError(saveserialize)}, status = status.HTTP_200_OK)
            else: 
                return Response({"status" : {"id" : ['id' +language.context[language.defaultLang]['missing']]}}, status=status.HTTP_200_OK)
...
class AllowancesMasterViews(APIView):
    ...
    def get(self, request, pk = None):
        ...
        try:
            ...
            if pk:
                list = models.AllowancesMaster.objects.filter(pk=pk).exclude(status='3').get()
                serializeobj = cSerializer.ListAllowancesMasterSerializer(list)
                return Response({"status": error.context['success_code'], "data": serializeobj.data}, status=status.HTTP_200_OK)
       
        except models.AllowancesMaster.DoesNotExist:
            return Response({"status" : error.context['error_code'], "message":"Status" +language.context[language.defaultLang]['dataNotFound'] }, status = status.HTTP_200_OK)

        lists = models.AllowancesMaster.objects.exclude(status='3')
        if normal_values:
            lists = lists.filter(reduce(operator.and_, 
                               (Q(**d) for d in [dict([i]) for i in normal_values.items()])))
        if array_values:
        ...
        serializer = cSerializer.ListAllowancesMasterSerializer(lists, many=True)
        return Response({"status":error.context['success_code'] , "data": serializer.data}, status=status.HTTP_200_OK)


class AllowancesMasterDetailViews(APIView):

    def get_object(self,pk):

        try:
            return models.AllowancesMaster.objects.get(pk = pk)
        except AllowancesMaster.DoesNotExist:
            raise Http404
    ...
    def post(self,request, pk = None):
        ...
        if 'name' not in request.data and request.data['status'] != 3:
            ...
            if 'id' in request.data:
                ...
                if not pk: 
                    duplicate_code = models.AllowancesMaster.objects.values('code').filter(code=request.data['code']).exclude(status=3)
                    duplicate_name = models.AllowancesMaster.objects.values('name').filter(name=request.data['name']).exclude(status=3)
                    ...
                    if duplicate_code:            
                        ...
                        saveserialize = cSerializer.AllowancesMasterSerializer(data = request.data)
                        if saveserialize.is_valid():
                            saveserialize.save()
                            return Response({"status" :error.context['success_code'], "message":"New AllowancesMaster" +language.context[language.defaultLang]['insert'], "data":saveserialize.data}, status=status.HTTP_200_OK)
                        else:
                            return Response({"status" :error.context['error_code'], "message": error.serializerError(saveserialize)}, status=status.HTTP_200_OK)

                else:
                    if request.data['status'] != 3:
                        duplicate_code = models.AllowancesMaster.objects.values('code').filter(code=request.data['code']).exclude(Q(id=request.data['id']) | Q(status=3))
                        duplicate_name = models.AllowancesMaster.objects.values('name').filter(name=request.data['name']).exclude(Q(id=request.data['id']) | Q(status=3))
                        if duplicate_code:            
                            return Response({"status" :error.context['error_code'], "message":language.context[language.defaultLang]['exit code'] },status=status.HTTP_200_OK)                    
                        elif duplicate_name:   
                            return Response({"status" :error.context['error_code'], "message":language.context[language.defaultLang]['exit name'] },status=status.HTTP_200_OK)                                            
                    ...
                    saveserialize = cSerializer.AllowancesMasterSerializer(list, data = request.data, partial= True)
                    if saveserialize.is_valid():
                        saveserialize.save()
                        return Response({"status" :error.context['success_code'], "message":"AllowancesMaster" +language.context[language.defaultLang]['update'], "data":saveserialize.data}, status=status.HTTP_200_OK)
                    else:
                        return Response({"status" :error.context['error_code'], "message":error.serializerError(saveserialize)}, status = status.HTTP_200_OK)
            else: 
                return Response({"status" : {"id" : ['id' +language.context[language.defaultLang]['missing']]}}, status=status.HTTP_200_OK)
...
#deductionsmaster
class DeductionsMasterViews(APIView):
    ...
    def get(self, request, pk = None):
        ...
        try:
            ...
            if pk:
                list = models.DeductionsMaster.objects.filter(pk=pk).exclude(status='3').get()
                serializeobj = cSerializer.ListDeductionsMasterSerializer(list)
                return Response({"status": error.context['success_code'], "data": serializeobj.data}, status=status.HTTP_200_OK)
       
        except models.DeductionsMaster.DoesNotExist:
            return Response({"status" : error.context['error_code'], "message":"Status" +language.context[language.defaultLang]['dataNotFound'] }, status = status.HTTP_200_OK)

        lists = models.DeductionsMaster.objects.exclude(status='3')
        if normal_values:
            lists = lists.filter(reduce(operator.and_, 
                               (Q(**d) for d in [dict([i]) for i in normal_values.items()])))
        if array_values:
        ...
        serializer = cSerializer.ListDeductionsMasterSerializer(lists, many=True)
        return Response({"status":error.context['success_code'] , "data": serializer.data}, status=status.HTTP_200_OK)

class DeductionsMasterDetailViews(APIView):

    def get_object(self,pk):

        try:
            return models.DeductionsMaster.objects.get(pk = pk)
        except DeductionsMaster.DoesNotExist:
            raise Http404
    ...
    def post(self,request, pk = None):
        ...
        if 'name' not in request.data and request.data['status'] != 3:
            ...
            if 'id' in request.data:
                ...
                if not pk: 
                    duplicate_code = models.DeductionsMaster.objects.values('code').filter(code=request.data['code']).exclude(status=3)
                    duplicate_name = models.DeductionsMaster.objects.values('name').filter(name=request.data['name']).exclude(status=3)
                    ...
                    if duplicate_code:            
                        ...
                        saveserialize = cSerializer.DeductionsMasterSerializer(data = request.data)
                        if saveserialize.is_valid():
                            saveserialize.save()
                            return Response({"status" :error.context['success_code'], "message":"New DeductionsMaster" +language.context[language.defaultLang]['insert'], "data":saveserialize.data}, status=status.HTTP_200_OK)
                        else:
                            return Response({"status" :error.context['error_code'], "message": error.serializerError(saveserialize)}, status=status.HTTP_200_OK)

                else:
                    if request.data['status'] != 3:
                        duplicate_code = models.DeductionsMaster.objects.values('code').filter(code=request.data['code']).exclude(Q(id=request.data['id']) | Q(status=3))
                        duplicate_name = models.DeductionsMaster.objects.values('name').filter(name=request.data['name']).exclude(Q(id=request.data['id']) | Q(status=3))
                        if duplicate_code:            
                            return Response({"status" :error.context['error_code'], "message":language.context[language.defaultLang]['exit code'] },status=status.HTTP_200_OK)                    
                        elif duplicate_name:   
                            return Response({"status" :error.context['error_code'], "message":language.context[language.defaultLang]['exit name'] },status=status.HTTP_200_OK)                                            
                    ...
                    saveserialize = cSerializer.DeductionsMasterSerializer(list, data = request.data, partial= True)
                    if saveserialize.is_valid():
                        saveserialize.save()
                        return Response({"status" :error.context['success_code'], "message":"DeductionsMaster" +language.context[language.defaultLang]['update'], "data":saveserialize.data}, status=status.HTTP_200_OK)
                    else:
                        return Response({"status" :error.context['error_code'], "message":error.serializerError(saveserialize)}, status = status.HTTP_200_OK)
            else: 
                return Response({"status" : {"id" : ['id' +language.context[language.defaultLang]['missing']]}}, status=status.HTTP_200_OK)
...
class PayGradeViews(APIView):
    ...
    def get(self, request, pk = None):
        ...
        try:
            ...
            if pk:
                list = models.PayGrade.objects.filter(pk=pk).exclude(status='3').get()
                serializeobj = cSerializer.ListPayGradeSerializer(list)
                return Response({"status": error.context['success_code'], "data": serializeobj.data}, status=status.HTTP_200_OK)
       
        except models.PayGrade.DoesNotExist:
            return Response({"status" : error.context['error_code'], "message":"Status" +language.context[language.defaultLang]['dataNotFound'] }, status = status.HTTP_200_OK)

        lists = models.PayGrade.objects.exclude(status='3')
        if normal_values:
            lists = lists.filter(reduce(operator.and_, 
                               (Q(**d) for d in [dict([i]) for i in normal_values.items()])))
        if array_values:
        ...
        serializer = cSerializer.ListPayGradeSerializer(lists, many=True)
        return Response({"status":error.context['success_code'] , "data": serializer.data}, status=status.HTTP_200_OK)


class PayGradeDetailViews(APIView):

    def get_object(self,pk):

        try:
            return models.PayGrade.objects.get(pk = pk)
    ...
    def post(self,request, pk = None):
        ...
        if 'name' not in request.data and request.data['status'] != 3:
            ...
            if 'id' in request.data:
                ...
                if not pk: 
                    duplicate_code = models.PayGrade.objects.values('code').filter(code=request.data['code']).exclude(status=3)
                    duplicate_name = models.PayGrade.objects.values('name').filter(name=request.data['name']).exclude(status=3)
                    ...
                    if duplicate_code:            
                        ...
                        saveserialize = cSerializer.PayGradeSerializer(data = request.data)
                        if saveserialize.is_valid():
                            saveserialize.save()
                            return Response({"status" :error.context['success_code'], "message":"New PayGrade" +language.context[language.defaultLang]['insert'], "data":saveserialize.data}, status=status.HTTP_200_OK)
                        else:
                            return Response({"status" :error.context['error_code'], "message": error.serializerError(saveserialize)}, status=status.HTTP_200_OK)

                else:
                    if request.data['status'] != 3:
                        duplicate_code = models.PayGrade.objects.values('code').filter(code=request.data['code']).exclude(Q(id=request.data['id']) | Q(status=3))
                        duplicate_name = models.PayGrade.objects.values('name').filter(name=request.data['name']).exclude(Q(id=request.data['id']) | Q(status=3))
                        if duplicate_code:            
                            return Response({"status" :error.context['error_code'], "message":language.context[language.defaultLang]['exit code'] },status=status.HTTP_200_OK)                    
                        elif duplicate_name:   
                            return Response({"status" :error.context['error_code'], "message":language.context[language.defaultLang]['exit name'] },status=status.HTTP_200_OK)                                            
                    ...
                    saveserialize = cSerializer.PayGradeSerializer(list, data = request.data, partial= True)
                    if saveserialize.is_valid():
                        saveserialize.save()
                        return Response({"status" :error.context['success_code'], "message":"PayGrade" +language.context[language.defaultLang]['update'], "data":saveserialize.data}, status=status.HTTP_200_OK)
                    else:
                        return Response({"status" :error.context['error_code'], "message":error.serializerError(saveserialize)}, status = status.HTTP_200_OK)
            else: 
...
class PersonnelTypeViews(APIView):
    ...
    def get(self, request, pk = None):
        ...
        try:
            ...
            if pk:
                list = models.PersonnelType.objects.filter(pk=pk).exclude(status='3').get()
                serializeobj = cSerializer.ListPersonnelTypeSerializer(list)
                return Response({"status": error.context['success_code'], "data": serializeobj.data}, status=status.HTTP_200_OK)
       
        except models.PersonnelType.DoesNotExist:
            return Response({"status" : error.context['error_code'], "message":"Status" +language.context[language.defaultLang]['dataNotFound'] }, status = status.HTTP_200_OK)

        lists = models.PersonnelType.objects.exclude(status='3')
        if normal_values:
            lists = lists.filter(reduce(operator.and_, 
                               (Q(**d) for d in [dict([i]) for i in normal_values.items()])))
        if array_values:
        ...
        serializer = cSerializer.ListPersonnelTypeSerializer(lists, many=True)
        return Response({"status":error.context['success_code'] , "data": serializer.data}, status=status.HTTP_200_OK)


class PersonnelTypeDetailViews(APIView):

    def get_object(self,pk):

        try:
            return models.PersonnelType.objects.get(pk = pk)
    ...
    def post(self,request, pk = None):
        ...
        if 'name' not in request.data and request.data['status'] != 3:
            ...
            if 'id' in request.data:
                ...
                if not pk: 
                    duplicate_code = models.PersonnelType.objects.values('code').filter(code=request.data['code']).exclude(status=3)
                    duplicate_name = models.PersonnelType.objects.values('name').filter(name=request.data['name']).exclude(status=3)
                    ...
                    if duplicate_code:            
                        ...
                        saveserialize = cSerializer.PersonnelTypeSerializer(data = request.data)
                        if saveserialize.is_valid():
                            saveserialize.save()
                            return Response({"status" :error.context['success_code'], "message":"New PersonnelType" +language.context[language.defaultLang]['insert'], "data":saveserialize.data}, status=status.HTTP_200_OK)
                        else:
                            return Response({"status" :error.context['error_code'], "message": error.serializerError(saveserialize)}, status=status.HTTP_200_OK)

                else:
                    if request.data['status'] != 3:
                        duplicate_code = models.PersonnelType.objects.values('code').filter(code=request.data['code']).exclude(Q(id=request.data['id']) | Q(status=3))
                        duplicate_name = models.PersonnelType.objects.values('name').filter(name=request.data['name']).exclude(Q(id=request.data['id']) | Q(status=3))
                        if duplicate_code:            
                            return Response({"status" :error.context['error_code'], "message":language.context[language.defaultLang]['exit code'] },status=status.HTTP_200_OK)                    
                        elif duplicate_name:   
                            return Response({"status" :error.context['error_code'], "message":language.context[language.defaultLang]['exit name'] },status=status.HTTP_200_OK)                                            
                    ...
                    saveserialize = cSerializer.PersonnelTypeSerializer(list, data = request.data, partial= True)
                    if saveserialize.is_valid():
                        saveserialize.save()
                        return Response({"status" :error.context['success_code'], "message":"PersonnelType" +language.context[language.defaultLang]['update'], "data":saveserialize.data}, status=status.HTTP_200_OK)
                    else:
                        return Response({"status" :error.context['error_code'], "message":error.serializerError(saveserialize)}, status = status.HTTP_200_OK)
            else: 
                return Response({"status" : {"id" : ['id' +language.context[language.defaultLang]['missing']]}}, status=status.HTTP_200_OK)
...
class RankViews(APIView):
    ...
    def get(self, request, pk = None):
        ...
        try:
            ...
            if pk:
                list = models.Rank.objects.filter(pk=pk).exclude(status='3').get()
                serializeobj = cSerializer.ListRankSerializer(list)
                return Response({"status": error.context['success_code'], "data": serializeobj.data}, status=status.HTTP_200_OK)
       
        except models.Rank.DoesNotExist:
            return Response({"status" : error.context['error_code'], "message":"Status" +language.context[language.defaultLang]['dataNotFound'] }, status = status.HTTP_200_OK)

        lists = models.Rank.objects.exclude(status='3')
        if normal_values:
            lists = lists.filter(reduce(operator.and_, 
                               (Q(**d) for d in [dict([i]) for i in normal_values.items()])))
        if array_values:
        ...
        serializer = cSerializer.ListRankSerializer(lists, many=True)
        return Response({"status":error.context['success_code'] , "data": serializer.data}, status=status.HTTP_200_OK)


class RankDetailViews(APIView):
    ...
    def post(self,request, pk = None):
        ...
        if 'name' not in request.data and request.data['status'] != 3:
            ...
            if 'id' in request.data:
                ...
                if not pk: 
                    duplicate_code = models.Rank.objects.values('code').filter(code=request.data['code']).exclude(status=3)
                    duplicate_name = models.Rank.objects.values('name').filter(name=request.data['name']).exclude(status=3)
                    ...
                    if duplicate_code:            
                        ...
                        saveserialize = cSerializer.RankSerializer(data = request.data)
                        if saveserialize.is_valid():
                            saveserialize.save()
                            return Response({"status" :error.context['success_code'], "message":"New Rank" +language.context[language.defaultLang]['insert'], "data":saveserialize.data}, status=status.HTTP_200_OK)
                        else:
                            return Response({"status" :error.context['error_code'], "message": error.serializerError(saveserialize)}, status=status.HTTP_200_OK)

                else:
                    if request.data['status'] != 3:
                        duplicate_code = models.Rank.objects.values('code').filter(code=request.data['code']).exclude(Q(id=request.data['id']) | Q(status=3))
                        duplicate_name = models.Rank.objects.values('name').filter(name=request.data['name']).exclude(Q(id=request.data['id']) | Q(status=3))
                        if duplicate_code:            
                            return Response({"status" :error.context['error_code'], "message":language.context[language.defaultLang]['exit code'] },status=status.HTTP_200_OK)                    
                        elif duplicate_name:   
                            return Response({"status" :error.context['error_code'], "message":language.context[language.defaultLang]['exit name'] },status=status.HTTP_200_OK)                                            
                    ...
                    saveserialize = cSerializer.RankSerializer(list, data = request.data, partial= True)
                    if saveserialize.is_valid():
                        saveserialize.save()
                        return Response({"status" :error.context['success_code'], "message":"Rank" +language.context[language.defaultLang]['update'], "data":saveserialize.data}, status=status.HTTP_200_OK)
                    else:
                        return Response({"status" :error.context['error_code'], "message":error.serializerError(saveserialize)}, status = status.HTTP_200_OK)
            else: 
                return Response({"status" : {"id" : ['id' +language.context[language.defaultLang]['missing']]}}, status=status.HTTP_200_OK)
...
# STATUSMASTER
class StatusMasterViews(APIView):
    ...
    def get(self, request, pk = None):
        ...
        try:
            ...
            if pk:
                list = models.StatusMaster.objects.filter(pk=pk).exclude(status='3').get()
                serializeobj = cSerializer.ListStatusMasterSerializer(list)
                return Response({"status": error.context['success_code'], "data": serializeobj.data}, status=status.HTTP_200_OK)
       
        except models.StatusMaster.DoesNotExist:
            return Response({"status" : error.context['error_code'], "message":"Status" +language.context[language.defaultLang]['dataNotFound'] }, status = status.HTTP_200_OK)

        lists = models.StatusMaster.objects.exclude(status='3')
        if normal_values:
            lists = lists.filter(reduce(operator.and_, 
                               (Q(**d) for d in [dict([i]) for i in normal_values.items()])))
        if array_values:
        ...
        serializer = cSerializer.ListStatusMasterSerializer(lists, many=True)
        return Response({"status":error.context['success_code'] , "data": serializer.data}, status=status.HTTP_200_OK)


class StatusMasterDetailViews(APIView):

    def get_object(self,pk):

        try:
            return models.StatusMaster.objects.get(pk = pk)
    ...
    def post(self,request, pk = None):
        ...
        if 'name' not in request.data and request.data['status'] != 3:
            ...
            if 'id' in request.data:
                ...
                if not pk: 
                    duplicate_code = models.StatusMaster.objects.values('code').filter(code=request.data['code']).exclude(status=3)
                    duplicate_name = models.StatusMaster.objects.values('name').filter(name=request.data['name']).exclude(status=3)
                    ...
                    if duplicate_code:            
                        ...
                        saveserialize = cSerializer.StatusMasterSerializer(data = request.data)
                        if saveserialize.is_valid():
                            saveserialize.save()
                            return Response({"status" :error.context['success_code'], "message":"New StatusMaster" +language.context[language.defaultLang]['insert'], "data":saveserialize.data}, status=status.HTTP_200_OK)
                        else:
                            return Response({"status" :error.context['error_code'], "message": error.serializerError(saveserialize)}, status=status.HTTP_200_OK)

                else:
                    if request.data['status'] != 3:
                        duplicate_code = models.StatusMaster.objects.values('code').filter(code=request.data['code']).exclude(Q(id=request.data['id']) | Q(status=3))
                        duplicate_name = models.StatusMaster.objects.values('name').filter(name=request.data['name']).exclude(Q(id=request.data['id']) | Q(status=3))
                        if duplicate_code:            
                            return Response({"status" :error.context['error_code'], "message":language.context[language.defaultLang]['exit code'] },status=status.HTTP_200_OK)                    
                        elif duplicate_name:   
                            return Response({"status" :error.context['error_code'], "message":language.context[language.defaultLang]['exit name'] },status=status.HTTP_200_OK)                                            
                    ...
                    saveserialize = cSerializer.StatusMasterSerializer(list, data = request.data, partial= True)
                    if saveserialize.is_valid():
                        saveserialize.save()
                        return Response({"status" :error.context['success_code'], "message":"StatusMaster" +language.context[language.defaultLang]['update'], "data":saveserialize.data}, status=status.HTTP_200_OK)
                    else:
                        return Response({"status" :error.context['error_code'], "message":error.serializerError(saveserialize)}, status = status.HTTP_200_OK)
            else: 
                return Response({"status" : {"id" : ['id' +language.context[language.defaultLang]['missing']]}}, status=status.HTTP_200_OK)
# Item type master
class ItemTypeViews(APIView):
    ...
    def get(self, request, pk = None):
        ...
        try:
            ...
            if pk:
                list = models.ItemType.objects.filter(pk=pk).exclude(status='3').get()
                serializeobj = cSerializer.ListItemTypeSerializer(list)
                return Response({"status": error.context['success_code'], "data": serializeobj.data}, status=status.HTTP_200_OK)
       
        except models.ItemType.DoesNotExist:
            return Response({"status" : error.context['error_code'], "message":"Status" +language.context[language.defaultLang]['dataNotFound'] }, status = status.HTTP_200_OK)

        lists = models.ItemType.objects.exclude(status='3')
        if normal_values:
            lists = lists.filter(reduce(operator.and_, 
                               (Q(**d) for d in [dict([i]) for i in normal_values.items()])))
        if array_values:
        ...
        serializer = cSerializer.ListItemTypeSerializer(lists, many=True)
        return Response({"status":error.context['success_code'] , "data": serializer.data}, status=status.HTTP_200_OK)


class ItemTypeDetailViews(APIView):

    def get_object(self,pk):

        try:
            return models.ItemType.objects.get(pk = pk)
    ...
    def post(self,request, pk = None):
        ...
        if 'name' not in request.data and request.data['status'] != 3:
            ...
            if 'id' in request.data:
                ...
                if not pk: 
                    duplicate_code = models.ItemType.objects.values('code').filter(code=request.data['code']).exclude(status=3)
                    duplicate_name = models.ItemType.objects.values('name').filter(name=request.data['name']).exclude(status=3)
                    ...
                    if duplicate_code:            
                        ...
                        saveserialize = cSerializer.ItemTypeSerializer(data = request.data)
                        if saveserialize.is_valid():
                            saveserialize.save()
                            return Response({"status" :error.context['success_code'], "message":"New ItemType" +language.context[language.defaultLang]['insert'], "data":saveserialize.data}, status=status.HTTP_200_OK)
                        else:
                            return Response({"status" :error.context['error_code'], "message": error.serializerError(saveserialize)}, status=status.HTTP_200_OK)

                else:
                    if request.data['status'] != 3:
                        duplicate_code = models.ItemType.objects.values('code').filter(code=request.data['code']).exclude(Q(id=request.data['id']) | Q(status=3))
                        duplicate_name = models.ItemType.objects.values('name').filter(name=request.data['name']).exclude(Q(id=request.data['id']) | Q(status=3))
                        if duplicate_code:            
                            return Response({"status" :error.context['error_code'], "message":language.context[language.defaultLang]['exit code'] },status=status.HTTP_200_OK)                    
                        elif duplicate_name:   
                            return Response({"status" :error.context['error_code'], "message":language.context[language.defaultLang]['exit name'] },status=status.HTTP_200_OK)                                            
                    ...
                    saveserialize = cSerializer.ItemTypeSerializer(list, data = request.data, partial= True)
                    if saveserialize.is_valid():
                        saveserialize.save()
                        return Response({"status" :error.context['success_code'], "message":"ItemType" +language.context[language.defaultLang]['update'], "data":saveserialize.data}, status=status.HTTP_200_OK)
                    else:
                        return Response({"status" :error.context['error_code'], "message":error.serializerError(saveserialize)}, status = status.HTTP_200_OK)
            else: 
                return Response({"status" : {"id" : ['id' +language.context[language.defaultLang]['missing']]}}, status=status.HTTP_200_OK)
...
# storage location
class StorageLocationViews(APIView):
    ...
    def get(self, request, pk = None):
        ...
        try:
            ...
            if pk:
                list = models.StorageLocation.objects.filter(pk=pk).exclude(status='3').get()
                serializeobj = cSerializer.ListStorageLocationSerializer(list)
                return Response({"status": error.context['success_code'], "data": serializeobj.data}, status=status.HTTP_200_OK)
       
        except models.StorageLocation.DoesNotExist:
            return Response({"status" : error.context['error_code'], "message":"Status" +language.context[language.defaultLang]['dataNotFound'] }, status = status.HTTP_200_OK)

        lists = models.StorageLocation.objects.exclude(status='3')
        if normal_values:
            lists = lists.filter(reduce(operator.and_, 
                               (Q(**d) for d in [dict([i]) for i in normal_values.items()])))
        if array_values:
        ...
        serializer = cSerializer.ListStorageLocationSerializer(lists, many=True)
        return Response({"status":error.context['success_code'] , "data": serializer.data}, status=status.HTTP_200_OK)


class StorageLocationDetailViews(APIView):

    def get_object(self,pk):

        try:
            return models.StorageLocation.objects.get(pk = pk)
    ...
    def post(self,request, pk = None):
        ...
        if 'name' not in request.data and request.data['status'] != 3:
            ...
            if 'id' in request.data:
                ...
                if not pk: 
                    duplicate_code = models.StorageLocation.objects.values('code').filter(code=request.data['code']).exclude(status=3)
                    duplicate_name = models.StorageLocation.objects.values('name').filter(name=request.data['name']).exclude(status=3)
                    ...
                    if duplicate_code:            
                        ...
                        saveserialize = cSerializer.StorageLocationSerializer(data = request.data)
                        if saveserialize.is_valid():
                            saveserialize.save()
                            return Response({"status" :error.context['success_code'], "message":"New StorageLocation" +language.context[language.defaultLang]['insert'], "data":saveserialize.data}, status=status.HTTP_200_OK)
                        else:
                            return Response({"status" :error.context['error_code'], "message": error.serializerError(saveserialize)}, status=status.HTTP_200_OK)

                else:
                    if request.data['status'] != 3:
                        duplicate_code = models.StorageLocation.objects.values('code').filter(code=request.data['code']).exclude(Q(id=request.data['id']) | Q(status=3))
                        duplicate_name = models.StorageLocation.objects.values('name').filter(name=request.data['name']).exclude(Q(id=request.data['id']) | Q(status=3))
                        if duplicate_code:            
                            return Response({"status" :error.context['error_code'], "message":language.context[language.defaultLang]['exit code'] },status=status.HTTP_200_OK)                    
                        elif duplicate_name:   
                            return Response({"status" :error.context['error_code'], "message":language.context[language.defaultLang]['exit name'] },status=status.HTTP_200_OK)                                            
                    ...
                    saveserialize = cSerializer.StorageLocationSerializer(list, data = request.data, partial= True)
                    if saveserialize.is_valid():
                        saveserialize.save()
                        return Response({"status" :error.context['success_code'], "message":"StorageLocation" +language.context[language.defaultLang]['update'], "data":saveserialize.data}, status=status.HTTP_200_OK)
                    else:
                        return Response({"status" :error.context['error_code'], "message":error.serializerError(saveserialize)}, status = status.HTTP_200_OK)
            else: 
                return Response({"status" : {"id" : ['id' +language.context[language.defaultLang]['missing']]}}, status=status.HTTP_200_OK)
...
class IssueViews(APIView):
    ...
    def get(self, request, pk = None):
        ...
        try:
            ...
            if pk:
                list = models.Issue.objects.filter(pk=pk).exclude(status='3').get()
                serializeobj = cSerializer.ListIssueSerializer(list)
                return Response({"status": error.context['success_code'], "data": serializeobj.data}, status=status.HTTP_200_OK)
       
        except models.Issue.DoesNotExist:
            return Response({"status" : error.context['error_code'], "message":"Status" +language.context[language.defaultLang]['dataNotFound'] }, status = status.HTTP_200_OK)

        lists = models.Issue.objects.exclude(status='3')
        if normal_values:
            lists = lists.filter(reduce(operator.and_, 
                               (Q(**d) for d in [dict([i]) for i in normal_values.items()])))
        if array_values:
        ...
        serializer = cSerializer.ListIssueSerializer(lists, many=True)
        return Response({"status":error.context['success_code'] , "data": serializer.data}, status=status.HTTP_200_OK)


class IssueDetailViews(APIView):

    def get_object(self,pk):

        try:
            return models.Issue.objects.get(pk = pk)
    ...
    def post(self,request, pk = None):
        ...
        #     return Response({"status":error.context['error_code'], "message" : "Code" +language.context[language.defaultLang]['missing'] },status=status.HTTP_200_OK)
            ...
            if 'id' in request.data:
                ...
                if not pk: 
                    # duplicate_code = models.Issue.objects.values('code').filter(code=request.data['code']).exclude(status=3)
                    duplicate_name = models.Issue.objects.values('name').filter(name=request.data['name']).exclude(status=3)
                    ...
                    if duplicate_name:   
                        ...
                        saveserialize = cSerializer.IssueSerializer(data = request.data)
                        if saveserialize.is_valid():
                            saveserialize.save()
                            return Response({"status" :error.context['success_code'], "message":"New Issue" +language.context[language.defaultLang]['insert'], "data":saveserialize.data}, status=status.HTTP_200_OK)
                        else:
                            return Response({"status" :error.context['error_code'], "message": error.serializerError(saveserialize)}, status=status.HTTP_200_OK)

                else:
                    if request.data['status'] != 3:
                        # duplicate_code = models.Issue.objects.values('code').filter(code=request.data['code']).exclude(Q(id=request.data['id']) | Q(status=3))
                        duplicate_name = models.Issue.objects.values('name').filter(name=request.data['name']).exclude(Q(id=request.data['id']) | Q(status=3))
                        # if duplicate_code:            
                        #     return Response({"status" :error.context['error_code'], "message":language.context[language.defaultLang]['exit code'] },status=status.HTTP_200_OK)                    
                        # elif duplicate_name:   
                        #     return Response({"status" :error.context['error_code'], "message":language.context[language.defaultLang]['exit name'] },status=status.HTTP_200_OK)                                            
                    ...
                    saveserialize = cSerializer.IssueSerializer(list, data = request.data, partial= True)
                    if saveserialize.is_valid():
                        saveserialize.save()
                        return Response({"status" :error.context['success_code'], "message":"Issue" +language.context[language.defaultLang]['update'], "data":saveserialize.data}, status=status.HTTP_200_OK)
                    else:
                        return Response({"status" :error.context['error_code'], "message":error.serializerError(saveserialize)}, status = status.HTTP_200_OK)
            else: 
                return Response({"status" : {"id" : ['id' +language.context[language.defaultLang]['missing']]}}, status=status.HTTP_200_OK)
...
class CourseViews(APIView):
    ...
    def get(self, request, pk = None):
        ...
        try:
            ...
            if pk:
                list = models.Course.objects.filter(pk=pk).exclude(status='3').get()
                serializeobj = cSerializer.ListCourseSerializer(list)
                return Response({"status": error.context['success_code'], "data": serializeobj.data}, status=status.HTTP_200_OK)
       
        except models.Course.DoesNotExist:
            return Response({"status" : error.context['error_code'], "message":"Status" +language.context[language.defaultLang]['dataNotFound'] }, status = status.HTTP_200_OK)

        lists = models.Course.objects.exclude(status='3')
        if normal_values:
            lists = lists.filter(reduce(operator.and_, 
                               (Q(**d) for d in [dict([i]) for i in normal_values.items()])))
        if array_values:
        ...
        serializer = cSerializer.ListCourseSerializer(lists, many=True)
        return Response({"status":error.context['success_code'] , "data": serializer.data}, status=status.HTTP_200_OK)
    ...
    def post(self,request, pk = None):
        ...
        if 'name' not in request.data and request.data['status'] != 3:
            ...
            if 'id' in request.data:
                ...
                if not pk: 
                    duplicate_code = models.Course.objects.values('code').filter(code=request.data['code']).exclude(status=3)
                    duplicate_name = models.Course.objects.values('name').filter(name=request.data['name']).exclude(status=3)
                    ...
                    if duplicate_code:            
                        ...
                        saveserialize = cSerializer.CourseSerializer(data = request.data)
                        if saveserialize.is_valid():
                            saveserialize.save()
                            return Response({"status" :error.context['success_code'], "message":"New Course" +language.context[language.defaultLang]['insert'], "data":saveserialize.data}, status=status.HTTP_200_OK)
                        else:
                            return Response({"status" :error.context['error_code'], "message": error.serializerError(saveserialize)}, status=status.HTTP_200_OK)

                else:
                    if request.data['status'] != 3:
                        duplicate_code = models.Course.objects.values('code').filter(code=request.data['code']).exclude(Q(id=request.data['id']) | Q(status=3))
                        duplicate_name = models.Course.objects.values('name').filter(name=request.data['name']).exclude(Q(id=request.data['id']) | Q(status=3))
                        if duplicate_code:            
                            return Response({"status" :error.context['error_code'], "message":language.context[language.defaultLang]['exit code'] },status=status.HTTP_200_OK)                    
                        elif duplicate_name:   
                            return Response({"status" :error.context['error_code'], "message":language.context[language.defaultLang]['exit name'] },status=status.HTTP_200_OK)                                            
                    ...
                    saveserialize = cSerializer.CourseSerializer(list, data = request.data, partial= True)
                    if saveserialize.is_valid():
                        saveserialize.save()
                        return Response({"status" :error.context['success_code'], "message":"Course" +language.context[language.defaultLang]['update'], "data":saveserialize.data}, status=status.HTTP_200_OK)
                    else:
                        return Response({"status" :error.context['error_code'], "message":error.serializerError(saveserialize)}, status = status.HTTP_200_OK)
            else: 
                return Response({"status" : {"id" : ['id' +language.context[language.defaultLang]['missing']]}}, status=status.HTTP_200_OK)
...
class HolidayViews(APIView):
    ...
    def get(self, request, pk = None):
        ...
        try:
            ...
            if pk:
                list = models.Holiday.objects.filter(pk=pk).exclude(status='3').get()
                serializeobj = cSerializer.ListHolidaySerializer(list)
                return Response({"status": error.context['success_code'], "data": serializeobj.data}, status=status.HTTP_200_OK)
       
        except models.Holiday.DoesNotExist:
            return Response({"status" : error.context['error_code'], "message":"Status" +language.context[language.defaultLang]['dataNotFound'] }, status = status.HTTP_200_OK)

        lists = models.Holiday.objects.exclude(status='3')
        if normal_values:
            lists = lists.filter(reduce(operator.and_, 
                               (Q(**d) for d in [dict([i]) for i in normal_values.items()])))
        if array_values:
        ...
        serializer = cSerializer.ListHolidaySerializer(lists, many=True)
        return Response({"status":error.context['success_code'] , "data": serializer.data}, status=status.HTTP_200_OK)


class HolidayDetailViews(APIView):

    def get_object(self,pk):

        try:
            return models.Holiday.objects.get(pk = pk)
    ...
    def post(self,request, pk = None):
        ...
        if 'name' not in request.data and request.data['status'] != 3:
            ...
            if 'id' in request.data:
                ...
                if not pk: 
                    duplicate_code = models.Holiday.objects.values('code').filter(code=request.data['code']).exclude(status=3)
                    duplicate_name = models.Holiday.objects.values('name').filter(name=request.data['name']).exclude(status=3)
                    ...
                    if duplicate_code:            
                        ...
                        saveserialize = cSerializer.HolidaySerializer(data = request.data)
                        if saveserialize.is_valid():
                            saveserialize.save()
                            return Response({"status" :error.context['success_code'], "message":"New Holiday" +language.context[language.defaultLang]['insert'], "data":saveserialize.data}, status=status.HTTP_200_OK)
                        else:
                            return Response({"status" :error.context['error_code'], "message": error.serializerError(saveserialize)}, status=status.HTTP_200_OK)

                else:
                    if request.data['status'] != 3:
                        duplicate_code = models.Holiday.objects.values('code').filter(code=request.data['code']).exclude(Q(id=request.data['id']) | Q(status=3))
                        duplicate_name = models.Holiday.objects.values('name').filter(name=request.data['name']).exclude(Q(id=request.data['id']) | Q(status=3))
                        if duplicate_code:            
                            return Response({"status" :error.context['error_code'], "message":language.context[language.defaultLang]['exit code'] },status=status.HTTP_200_OK)                    
                        elif duplicate_name:   
                            return Response({"status" :error.context['error_code'], "message":language.context[language.defaultLang]['exit name'] },status=status.HTTP_200_OK)                                            
                    ...
                    saveserialize = cSerializer.HolidaySerializer(list, data = request.data, partial= True)
                    if saveserialize.is_valid():
                        saveserialize.save()
                        return Response({"status" :error.context['success_code'], "message":"Holiday" +language.context[language.defaultLang]['update'], "data":saveserialize.data}, status=status.HTTP_200_OK)
                    else:
                        return Response({"status" :error.context['error_code'], "message":error.serializerError(saveserialize)}, status = status.HTTP_200_OK)
            else: 
                return Response({"status" : {"id" : ['id' +language.context[language.defaultLang]['missing']]}}, status=status.HTTP_200_OK)
...
# stockregister
class StockRegisterViews(APIView):
    ...
    def get(self, request, pk = None):
        ...
        try:
            ...
            if pk:
                list = models.StockRegister.objects.filter(pk=pk).exclude(status='3').get()
                serializeobj = cSerializer.ListStockRegisterSerializer(list)
                return Response({"status": error.context['success_code'], "data": serializeobj.data}, status=status.HTTP_200_OK)
       
        except models.StockRegister.DoesNotExist:
            return Response({"status" : error.context['error_code'], "message":"Status" +language.context[language.defaultLang]['dataNotFound'] }, status = status.HTTP_200_OK)

        lists = models.StockRegister.objects.exclude(status='3')
        if normal_values:
            lists = lists.filter(reduce(operator.and_, 
                               (Q(**d) for d in [dict([i]) for i in normal_values.items()])))
        if array_values:
        ...
        serializer = cSerializer.ListStockRegisterSerializer(lists, many=True)
        return Response({"status":error.context['success_code'] , "data": serializer.data}, status=status.HTTP_200_OK)


class StockRegisterDetailViews(APIView):

    def get_object(self,pk):

        try:
            return models.StockRegister.objects.get(pk = pk)
        ...
        if 'name' not in request.data and request.data['status'] != 3:
            ...
            if 'id' in request.data:
                ...
                if not pk: 
                    ...
                    duplicate_name = models.StockRegister.objects.values('name').filter(name=request.data['name']).exclude(status=3)
                    ...
                    if duplicate_name:   
                        ...
                        saveserialize = cSerializer.StockRegisterSerializer(data = request.data)
                        if saveserialize.is_valid():
                            saveserialize.save()
                            return Response({"status" :error.context['success_code'], "message":"New StockRegister" +language.context[language.defaultLang]['insert'], "data":saveserialize.data}, status=status.HTTP_200_OK)
                        else:
                            return Response({"status" :error.context['error_code'], "message": error.serializerError(saveserialize)}, status=status.HTTP_200_OK)
                    ...
                    if request.data['status'] != 3:
                        ...
                        duplicate_name = models.StockRegister.objects.values('name').filter(name=request.data['name']).exclude(Q(id=request.data['id']) | Q(status=3))
                        if duplicate_code:            
                            return Response({"status" :error.context['error_code'], "message":language.context[language.defaultLang]['exit code'] },status=status.HTTP_200_OK)                    
                        elif duplicate_name:   
                            return Response({"status" :error.context['error_code'], "message":language.context[language.defaultLang]['exit name'] },status=status.HTTP_200_OK)                                            
                    ...
                    saveserialize = cSerializer.StockRegisterSerializer(list, data = request.data, partial= True)
                    if saveserialize.is_valid():
                        saveserialize.save()
                        return Response({"status" :error.context['success_code'], "message":"StockRegister" +language.context[language.defaultLang]['update'], "data":saveserialize.data}, status=status.HTTP_200_OK)
                    else:
                        return Response({"status" :error.context['error_code'], "message":error.serializerError(saveserialize)}, status = status.HTTP_200_OK)
            else: 
                return Response({"status" : {"id" : ['id' +language.context[language.defaultLang]['missing']]}}, status=status.HTTP_200_OK)
...
class StockLogDetailViews(APIView):
    ...
    def get_object(self,pk):
        ...
        try:
            return models.StockLog.objects.get(pk = pk)
